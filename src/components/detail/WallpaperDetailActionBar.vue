<template>
  ...
  >
    <button
      type="button"
      class="inline-flex flex-1 items-center justify-center gap-2 border font-semibold tracking-tight backdrop-blur-xl transition disabled:cursor-not-allowed disabled:opacity-55"
      :class="[
        compact ? 'min-h-[38px] rounded-xl px-3 text-[13px]' : 'min-h-[48px] rounded-2xl px-4 text-[15px]',
        themeLight
      ...
      @click="$emit('download')"
    >
      <span :class="compact ? 'text-sm leading-none' : 'text-lg leading-none'" aria-hidden="true">↓</span>
    ...
    >
      <span :class="compact ? 'text-sm leading-none' : 'text-lg leading-none'" aria-hidden="true">{{ favorited ? '★' : '☆' }}</span>
